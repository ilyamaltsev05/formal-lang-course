...
class TestIsomorphismAfterDump:
    def setup_method(self):
        ...
        tsk01.dump_two_cycles_graph(
            10, 100, ("first label", "second label"), Path("./tmp/10x100.dot")
        )

    def reference_graph(
        self, first_cycle_size: int, second_cycle_size: int, labels: tuple[str, str]
    ) -> nx.Graph:
        c1 = nx.cycle_graph(first_cycle_size + 1)
        c2 = nx.cycle_graph(
            [
                i + first_cycle_size if i != 0 else 0
                for i in range(second_cycle_size + 1)
            ]
        )
